...
Hank`);
  ...
  const payoutPreview = useMemo(() => computePayouts({
  ...
  const state = encodeURIComponent(
    Buffer.from(JSON.stringify({ name, format, buybacks, buybackFee, players })).toString("base64")
  );
  ...
  return (
    <main className="mx-auto max-w-6xl px-6 py-10 space-y-8">
      <div className="flex items-center justify-between">
        ...
        <Link href={`/${params.tenantId}/tournament/preview?state=${state}`}
      ...
      <div className="grid md:grid-cols-2 gap-6">
        ...
        <div className="rounded-2xl border border-slate-800 bg-slate-900/40 p-6 space-y-4">
          <div>
            ...
            <input className="mt-1 w-full rounded-xl bg-slate-800 border border-slate-700 px-4 py-2"
          ...
          <div className="grid grid-cols-3 gap-3">
            <div>
              ...
              <input type="number" className="mt-1 w-full rounded-xl bg-slate-800 border border-slate-700 px-3 py-2"
                     value={entryFee} onChange={(e) => setEntryFee(Number(e.target.value))} />
            </div>
            <div>
              <label className="text-sm text-slate-300">Greens fee</label>
              <input type="number" className="mt-1 w-full rounded-xl bg-slate-800 border border-slate-700 px-3 py-2"
                     value={greenFee} onChange={(e) => setGreenFee(Number(e.target.value))} />
            </div>
            <div>
              <label className="text-sm text-slate-300">Sponsor add</label>
              <input type="number" className="mt-1 w-full rounded-xl bg-slate-800 border border-slate-700 px-3 py-2"
                     value={sponsorAdd} onChange={(e) => setSponsorAdd(Number(e.target.value))} />
            </div>
          </div>
          ...
          <div className="grid grid-cols-2 gap-3">
            <div>
              ...
              <select className="mt-1 w-full rounded-xl bg-slate-800 border border-slate-700 px-3 py-2"
                      value={format} onChange={(e) => setFormat(e.target.value as "single"|"double")}>
            ...
            <div>
              ...
              <select className="mt-1 w-full rounded-xl bg-slate-800 border border-slate-700 px-3 py-2"
                      value={templateKey} onChange={(e) => setTemplateKey(e.target.value as any)}>
          ...
          <div className="flex items-center gap-3">
            <label className="inline-flex items-center gap-2">
              <input type="checkbox" checked={buybacks} onChange={(e)=>setBuybacks(e.target.checked)} />
              <span className="text-sm text-slate-300">Enable buybacks</span>
            </label>
            {buybacks && (
              <input type="number" className="rounded-xl bg-slate-800 border border-slate-700 px-3 py-2"
                     value={buybackFee} onChange={(e)=>setBuybackFee(Number(e.target.value))} />
            )}
          </div>
        </div>
        ...
        <div className="rounded-2xl border border-slate-800 bg-slate-900/40 p-6">
          <label className="text-sm text-slate-300">Players (one per line or comma-separated)</label>
          <textarea className="mt-1 h-64 w-full rounded-xl bg-slate-800 border border-slate-700 px-3 py-2"
      ...
      <div className="rounded-2xl border border-slate-800 bg-slate-900/40 p-6">
        ...
        <ul className="mt-4 grid md:grid-cols-2 gap-3">
          {payoutPreview.splits.map((s) => (
            <li key={s.place} className="rounded-lg bg-slate-800 px-4 py-2 flex items-center justify-between">
              <span className="text-slate-300">Place {s.place}</span>
              <span className="font-semibold">${s.amount.toFixed(2)} <span className="text-slate-400 text-sm">({(s.share*100).toFixed(0)}%)</span></span>
            </li>
          ))}
        </ul>
